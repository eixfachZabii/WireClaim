...
from src.evidence.policy.coverage import (
    DEFAULT_P_COVERED,
    LIMIT_COLLAPSE,
...
class PromptTests(unittest.TestCase):
    ...
    def test_the_policy_is_sliced_before_it_is_sent(self) -> None:
        ...
        with patch("src.evidence.policy.coverage.get_llm_client", return_value=client):
...
class AssessCoverageTests(unittest.TestCase):
    ...
    def test_every_sample_is_requested_and_averaged(self) -> None:
        ...
        with patch("src.evidence.policy.coverage.SAMPLES", 2), patch(
            "src.evidence.policy.coverage.get_llm_client", return_value=client
        ):
            verdicts = asyncio.run(assess_coverage(self.case))
    ...
    def test_a_long_invoice_is_split_into_several_calls(self) -> None:
        ...
        with patch("src.evidence.policy.coverage.SAMPLES", 1), patch(
            "src.evidence.policy.coverage.get_llm_client", return_value=client
        ):
