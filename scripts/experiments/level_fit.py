...
3. **The euro sweep and a held-out check.** Every candidate `(c0, c1)` is applied to the
   blended ensemble evidence, repriced through the shipped `src/pricing/engine.py`, and scored with
...
def nets(loaded: list[tuple], c0: float, c1: float, sigma_floor: float = 0.0) -> dict[int, float]:
    """Net per Game under one candidate map, priced by the shipped `src/pricing/engine.py`."""
    out: dict[int, float] = {}
