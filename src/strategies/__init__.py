...
(`fast_path.py`), the active estimator (`strategy2/`), its coherent Fair-Value comparison
(`strategy5/`), and the lower-priority tail-aware comparison (`strategy4/`). Strategy 1 and
Strategy 3 live in `src.legacy`.
...
#:
#: Strategy 2 remains top and is the only Proposal the live router normally submits.
#: Strategy 5 is immediately below it and Strategy 4 below that; both are recorded as
#: counterfactuals after Strategy 2 has already won, so experiments accumulate live evidence
#: without changing a Game.
